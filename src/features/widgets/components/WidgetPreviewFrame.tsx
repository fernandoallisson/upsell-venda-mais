...
const WidgetPreviewFrame = ({ viewport, children, fullscreen = false }: Props) => {
  if (viewport === 'mobile') {
    return (
      <div className={`mx-auto rounded-[2rem] border-8 border-slate-900 bg-slate-950 p-2 shadow-2xl ${fullscreen ? 'w-[360px]' : 'w-[280px]'}`}>
        ...
        <div className="min-h-[380px] overflow-hidden rounded-[1.4rem] bg-slate-100 p-3">
  ...
  return (
    <div className="mx-auto w-full max-w-2xl rounded-2xl border border-slate-300 bg-white shadow-2xl">
      <div className="flex items-center gap-2 border-b border-slate-200 px-4 py-3">
        <span className="h-3 w-3 rounded-full bg-rose-300" />
        <span className="h-3 w-3 rounded-full bg-amber-300" />
        <span className="h-3 w-3 rounded-full bg-emerald-300" />
      </div>
      <div className={`bg-slate-100 p-5 ${fullscreen ? 'min-h-[420px]' : 'min-h-[220px]'}`}>
